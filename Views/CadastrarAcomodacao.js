...
export default function CadastroAcomodacao({ route }) {
  ...
  async function registerAcomodacao() {
    if (!acomodacao || !alunoId || !dataInicio || !dataFim) {
      setErrorMessage('Por favor, preencha todos os campos.');
      return;
    }

    try {
      let response = await fetch(config.urlRootNode + 'createAcomodacao', {
        method: 'POST',
        headers: {
...
return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      <View style={styles.container}>
        <Text style={styles.title}>Cadastro de Acomodação</Text>
        <TextInput
          style={styles.input}
          value={acomodacao}
          onChangeText={setAcomodacao}
